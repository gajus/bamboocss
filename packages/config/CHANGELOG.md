...
- 100becd: Point `@bamboocss/types` at the declarations it ships. Since the move to tsdown, `main` and the `types` and
  `default` conditions named `dist/index.d.cts`, but this package is not built by tsdown: its build copies each source
  file to `dist/*.d.ts`, so no published tarball has contained the file its entry named.
  ...
  `@bamboocss/config` built its own declarations against the same entry, and two return types came out naming a type
  they never imported: `loadConfig` now returns the `LoadConfigResult` from `@bamboocss/types`, and `mergeHooks` returns
