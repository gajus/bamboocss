...
- bf68418: Fold `light-dark()` at a token's color component instead of its whole value.
  ...
  The two arms are now compared component by component and only the parts that differ are folded, which keeps the
  ...
  A part is only folded when it is provably a color: a hex, a color function, a color keyword, or a `var()` naming a
  token from the `colors` category. Anything else keeps its `@media (prefers-color-scheme: dark)` block, as do arms
  whose list or component counts disagree. A `colors` token still folds whole, so a raw `var()` pointed at a property
  bamboo never emitted is unaffected.
