...
function extract(files: Record<string, string>, entry: string) {
  ...
  const data = parseFile(ctx, entry)
...
describe('cross-file: tsconfig path aliases', () => {
  test('folds a value imported through a tsconfig path alias', () => {
    ...
    const data = parseFile(ctx, '/app/src/app.tsx')!
...
describe('cross-file: renaming re-exports', () => {
  ...
  /**
   * The test above, as valid ES. There, the explicit `export { p as q }` shadows the star
   * export's `q` — and `./b` exports no `p` — so the module fails to link at runtime and
   * TypeScript rejects the import. This pins the same property, a cycle guard keyed on the file
   * and the name together, on a graph a runtime accepts: `c.ts` is searched for `q` through
   * `./b`, then again for `s` through `a.ts`'s own star export.
   */
  test('a file searched for one name is still searched for another, on a graph that links', () => {
    expect(
      extract(
        {
          'app/src/a.ts': `export { q as r } from './b'
           export * from './c'`,
          'app/src/b.ts': `export * from './c'`,
          'app/src/c.ts': `export const q = { color: 'red.500' }
           export const s = { padding: '2' }`,
          'app/src/app.tsx': `${CSS_IMPORT}
           import { r, s } from './a'
           export const App = () => <div className={css(r, s, { margin: '2' })} />`,
        },
        'app/src/app.tsx',
      ),
    ).toEqual([[{ color: 'red.500' }, { padding: '2' }, { margin: '2' }]])
  })
})
