...
import { itemSet, parseAndExtract } from './fixture'
...
describe('pattern.raw() extraction in selectors', () => {
  ...
  /**
   * The `.raw` calls come out in source order now. The TypeScript engine grouped them by
   * pattern, which put every `center` before every `grid`; the only stylesheet effect is where
   * `.d_flex` sits beside `.d_grid` inside one sublayer — two atoms that never share an element,
   * since each belongs to a different pattern's base.
   */
  ...
  /**
   * Two sources above imported a binding twice — `import { flex, flex }` and
   * `import { flex, center, flex }` — as the retired suite had them. A duplicate binding is an
   * error TypeScript reports (TS2300) and the TypeScript engine extracted through it; the native
   * engine fails the whole file on it. Expected: the same items as the deduplicated import.
   */
  test.each([
    [
      'pattern function called inside another pattern',
      `import { flex, flex } from "styled-system/patterns";
      const layout = flex({ direction: 'column', '&:hover': { ...flex.raw({ gap: '4', direction: 'row' }), opacity: 0.9 } })`,
      'import { flex, flex }',
      'import { flex }',
    ],
    [
      'multiple pattern.raw spreads in same object',
      `import { css } from "styled-system/css";
      import { flex, center, flex } from "styled-system/patterns";
      const styles = css({ '&:hover': { ...flex.raw({ direction: 'row' }), ...center.raw({ inline: true }), color: 'red' } })`,
      'import { flex, center, flex }',
      'import { flex, center }',
    ],
  ])('%s — original source, with its duplicate import', (_name, malformed, from, to) => {
    const corrected = malformed.replace(from, to)

    expect(itemSet(parseAndExtract(malformed).json)).toEqual(itemSet(parseAndExtract(corrected).json))
  })
})
