...
`
  ...
  test('vue 2 Options API template', () => {
    ...
    // The component never imports `css`, so the calls are not bamboo's — the TypeScript engine
    // matched the bare name, the native engine follows the binding. Adding the import is what
    // makes them extract, which the tests above pin.
    expect(parseAndExtract(transformed).json).toEqual([])
    expect(
      parseAndExtract(`import { css } from 'styled-system/css'\n${transformed}`).json.map((item) => item.data),
    ).toEqual([
      [{ color: 'green.400' }],
      [{ color: 'purple.400' }],
      [{ color: 'red.500' }],
