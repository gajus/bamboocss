import { isStaticCompilerActive, markStaticCompilerActive } from "./static-compiler.mjs";
import { createRequire } from "node:module";
import { colors, logger, quote } from "@bamboocss/logger";
import { convertTsPathsToRegexes, diffConfigs, findConfig, loadConfig, mergeHooks } from "@bamboocss/config";
import { BambooError, cssVarRefs, getOrCreateSet, groupBy, truncateList, uniq } from "@bamboocss/shared";
import { appendFileSync, existsSync, readFileSync, statSync, writeFileSync } from "fs";
import { dirname, extname, isAbsolute, join, normalize, parse, relative, resolve, sep } from "path";
...
import { checkNamingAgreement, formatNamingDisagreement, messages, prunesPreflight } from "@bamboocss/core";
import { Generator, generatePackageExports } from "@bamboocss/generator";
import fs, { existsSync as existsSync$1, promises } from "node:fs";
import path, { dirname as dirname$1, isAbsolute as isAbsolute$1, join as join$1, resolve as resolve$1 } from "node:path";
...
const localTargetOf = (specifier, importer, options) => {
	const bases = [];
	if (specifier.startsWith(".")) bases.push(resolve$1(dirname$1(importer), specifier));
	else if (isAbsolute$1(specifier)) bases.push(specifier);
	else {
		for (const [pattern, targets] of Object.entries(options.paths ?? {})) {
			const star = pattern.indexOf("*");
			if (star === -1) {
				if (pattern !== specifier) continue;
				for (const target of targets) bases.push(resolve$1(options.cwd, target));
				continue;
			}
			...
			for (const target of targets) bases.push(resolve$1(options.cwd, target.replace("*", filled)));
		}
		if (options.baseUrl) bases.push(resolve$1(options.cwd, options.baseUrl, specifier));
	}
	const exists = options.fileExists ?? existsSync$1;
...
async function findClosestTsconfig(filename, root, configName = "tsconfig.json") {
	...
	for (;;) {
		...
		if (dir === resolvedRoot || path.dirname(dir) === dir) return null;
...
const resolve2posix = IS_POSIX ? (dir, filename) => dir ? path.resolve(dir, filename) : path.resolve(filename) : (dir, filename) => {
	const posix2native = (f) => f.replace(POSIX_SEP_RE, path.sep);
	return native2posix(dir ? path.resolve(posix2native(dir), posix2native(filename)) : path.resolve(posix2native(filename)));
};
...
const getTsConfigResolutionFiles = (conf) => resolutionFilesByConfig.get(conf) ?? Object.freeze(conf.tsconfigFile ? [path.resolve(conf.tsconfigFile)] : []);
const referencedTsconfigPath = (reference, rootPath) => path.resolve(path.dirname(rootPath), reference.endsWith(".json") ? reference : path.join(reference, "tsconfig.json"));
...
const collectReferenceGraph = async (rootPath, rootParsed, parseTsconfig, cache) => {
	...
	for (let index = 0; index < queue.length; index++) {
		...
		for (const reference of current.parsed.references ?? []) {
			...
			try {
				const source = await promises.readFile(referencedPath, "utf8");
...
async function loadTsConfig(conf, cwd, provenance, resolutionFiles) {
	...
	const { tsconfig, tsconfigFile: effectiveTsconfigPath } = await resolveSolutionTsconfigForFile(path.resolve(conf.path), tsconfigFile, rootParsed, gtc, cache);
	...
	if (resolutionFiles) resolutionFiles.value = Object.freeze([...new Set(readPathsFromCache(cache).map((file) => path.resolve(file)))].sort());
	if (compilerOptions?.paths) {
		const baseUrl = compilerOptions.baseUrl;
		result.tsOptions = {
			baseUrl,
			pathMappings: convertTsPathsToRegexes(compilerOptions.paths, resolveBaseUrlForCompilerOptions(baseUrl, effectiveTsconfigPath, cwd))
		};
	}
	if (provenance) {
...
var SourceProject = class {
	...
	*/
	...
	absolute = (filePath) => isAbsolute$1(filePath) ? filePath : join$1(this.options.cwd ?? process.cwd(), filePath);
...
function globDirname(globs) {
	...
	for (const glob of globs) {
		...
		if (nonGlobTokens?.length) rootDirs.add(join(...nonGlobTokens.map((token) => token.value)));
	}
...
const nodeRuntime = {
	...
	fs: {
		...
		glob(opts) {
			...
			return sortSources(glob.sync(opts.include, {
				cwd: opts.cwd,
		...
		watch(options) {
			...
			const isValidPath = picomatch(include, {
				cwd,
				ignore: exclude
			});
...
var BambooContext = class extends Generator {
	runtime;
	project;
	output;
	diff;
	...
	*/
	...
	constructor(conf) {
		...
		if (disagreement) throw new BambooError("NAMING_DISAGREEMENT", formatNamingDisagreement(disagreement));
	}
	/**
	...
	*/
	assertNoDeadCalls = (files) => {
		...
		const detail = truncateList(Array.from(groupBy(occurrences, ({ call }) => `${call.entrypoint}\0${call.mod}\0${call.name}\0${call.alias}`), ([, group]) => {
			const { call } = group[0];
	...
	*/
	...
	loadNativeExtractor = () => {
		...
		try {
			...
			if (!binary) throw new BambooError("NATIVE_EXTRACTION", `Native extraction is not available for ${process.platform}-${process.arch}.`, { cause: workspaceError });
			try {
				return nativeRequire(join$1(dirname$1(fileURLToPath(import.meta.url)), "native", binary));
			} catch (publishedError) {
				throw new BambooError("NATIVE_EXTRACTION", `Failed to load the native extractor binary ${binary}.`, { cause: publishedError });
			}
		}
	};
	...
	parseNativeFile = (filePath, native, encoder) => {
		if (native.errors.length) throw new BambooError("NATIVE_EXTRACTION", `Native extraction failed for ${filePath}:\n${native.errors.join("\n")}`);
	...
	*/
	extractableFiles = (files) => {
		...
		logger.debug("file:extract", `${extractable.length} of ${files.length} file(s) can reach a bamboo entrypoint; ${files.length - extractable.length} cannot and are not parsed; ${auxiliary.length} local import target(s) sit outside the inventory`);
		measure();
		return extractable;
	};
	...
	watchFiles = (cb, opts) => {
		...
		this.runtime.fs.watch({
			...
			poll,
			cwd
		}).on("all", debounce(async (event, file) => {
...
async function prepareNodeConfig(options = {}) {
	...
	conf.hooks = mergeHooks([...autoPlugins, {
...
function pruneTokensForBuild(ctx, sheet, results, sourceScans) {
	...
	if (failing.length && unresolved !== "off") {
		...
		if (unresolved === "error") throw new BambooError("TOKEN_REFERENCE_UNRESOLVED", where + "`prune.unresolvedPath: 'error'` asserts that every token path resolves at build time. Spell the path as a string literal at the call, give a template a static prefix so it can be bounded, name the category they land in with `prune: { keepTokens: ['colors.*'] }` and drop to `prune: { unresolvedPath: 'warn' }`, or set `prune: { unresolvedPath: 'off' }` to stop asserting.");
...
function declaredKeeps(ctx) {
	...
	for (const token of ctx.tokens.allTokens) {
		...
		for (const pattern of includes) if (isMatch(token.name, pattern, { caseSensitive: true })) used.add(pattern);
	}
...
var Builder = class {
	...
	*/
	isPotentialSourceFile = (filePath) => {
		...
		if (!this.sourceMatchers.has(ctx)) {
			...
			this.sourceMatchers.set(ctx, sourcePatterns.length ? picomatch(sourcePatterns, { ignore: globIgnore(ctx.config.exclude) }) : void 0);
		}
	...
	isPotentialConfigDependency = (filePath) => {
		...
		const matcher = patterns.length ? picomatch(patterns) : void 0;
	...
	recordConfigDependencies = (ctx, cwd) => {
		...
		for (const file of uniq([...ctx.conf.dependencies, ...ctx.explicitDeps])) this.configDependencies.add(resolve(root, file));
	};
	setup = async (options = {}) => {
		...
		*/
		...
		const explicitDeps = options.sourceChanges?.needsConfigReload ? uniq([...previousExplicitDeps, ...this.context.explicitDeps]) : this.context.explicitDeps;
	...
	configGraphFiles = (configPath) => uniq([
	...
	configGraphUnchanged = (configPath, sourceChanges) => {
		...
		const changed = sourceChanges ? new Set(sourceChanges.files.map((file) => this.sourcePath(resolve(this.context?.config.cwd ?? "", file)))) : void 0;
	...
	*/
	invalidateChangedSources = (ctx, inventory, seededAffected = /* @__PURE__ */ new Set(), previousLedger, capturedDependents = /* @__PURE__ */ new Map()) => {
		...
		for (const file of changed) if (!existsSync(file)) {
	...
	*/
	...
	getContextOrThrow = () => {
		if (!this.context) throw new BambooError("NO_CONTEXT", "context not loaded");
		return this.context;
	};
	...
	extractFile = (ctx, file) => {
		...
		if (parserResult) {
			const readSet = {
				dependencies: uniq(parserResult.getDependencies()).sort(),
				pendingCandidates: uniq([...parserResult.nativePendingCandidates]).sort()
			};
			const currentReads = readSet.dependencies;
			const currentConfigurations = uniq([...currentReads.length || readSet.pendingCandidates.length ? this.tsconfigResolutionFiles : [], ...parserResult.nativeConfigurationFiles]).sort();
			...
			const pending = meta.isUnchanged ? uniq([...previousPending, ...previousReads]).filter((dependency) => !existsSync(dependency) && !currentReads.includes(dependency) && (!previousCandidates.includes(dependency) || readSet.pendingCandidates.includes(dependency))).sort() : [];
...
const cssgen = async (ctx, options) => {
	...
	if (type) {
		...
		if (type === "preflight" && prunesPreflight(ctx.config.preflight)) ctx.prunePreflight(sheet, collectRenderedElements(ctx));
		if (outfile) {
...
async function debug(ctx, options) {
	...
	files.map((file) => {
		...
		if (outdir) {
			...
			return Promise.allSettled([fs.writeFile(`${outdir}${path.sep}${astJsonPath}`, JSON.stringify(result.toJSON(), null, 2)), fs.writeFile(`${outdir}${path.sep}${cssPath}`, css)]);
		}
	});
	logger.info("cli", `Found ${colors.bold(`${filesWithCss.length}/${files.length}`)} files using Bamboo`);
	measureTotal();
...
async function build(ctx, sourceScanCache, artifactIds) {
	...
	if (ctx.config.emitTokensOnly) return logger.info("css:emit", "Successfully rebuilt the css variables and js function to query your tokens ✨");
...
function setupGitIgnore(ctx) {
  ...
  `;
	...
	if (!file) return writeFileSync(".gitignore", txt);
	if (!readFileSync(file, "utf-8").includes(outdir)) appendFileSync(file, txt);
}
//#endregion
...
async function setupConfig(cwd, opts = {}) {
	...
	try {
		...
		if (!(err instanceof BambooError)) throw err;
	}
	const { detect } = await import("package-manager-detector");
	const pm = ((await detect({ cwd }))?.agent ?? "npm").split("@")[0];
	const cmd = pm === "npm" ? "npm run" : pm;
	const file = lookItUpSync("tsconfig.json", cwd) ? "bamboo.config.ts" : "bamboo.config.mjs";
	logger.info("init:config", `creating bamboo config file: ${quote(file)}`);
	if (!force && configFile) logger.warn("init:config", messages.configExists(cmd));
	else {
...
const findViteConfig = (cwd) => VITE_CONFIG_FILES.map((file) => join(cwd, file)).find((file) => existsSync(file));
/**
...
//#endregion
export { BambooContext, Builder, ParserResult, SourceProject, analyze, buildInfo, codegen, cssgen, debug, findViteConfig, generate, generatePackageExports, hasUncompilableSources, isStaticCompilerActive, loadConfigAndCreateContext, markStaticCompilerActive, setLogStream, setupConfig, setupGitIgnore, spec, startProfiling };
