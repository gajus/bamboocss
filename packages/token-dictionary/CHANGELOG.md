...
- 98b77a1: Fail the build when two tokens hash to the same css variable, instead of silently dropping one.
  ...
  Only reachable under `hash: true` or `hash: { cssVar: true }`. Unhashed variable names carry the token path, so they
