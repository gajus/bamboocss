...
- Updated dependencies [2a070c4]
  - @bamboocss/core@1.56.1
  - @bamboocss/node@1.56.1
  - @bamboocss/config@1.56.1
...
- 8097840: Stop Rolldown logging a bundle-assignment warning on every build.

  After pruning renames the stylesheet, the asset is also re-keyed in the bundle so a plugin that looks it up by its new
  name finds it — `@vitejs/plugin-rsc` does exactly that. Rolldown does not support the assignment: it ignores it and
  logs `[plugin bamboocss:css-early] Error: This plugin assigns to bundle variable … This will be ignored` once per
  build. The message says `Error`, names Bamboo, and describes a write that was already being discarded, so it read as a
  Bamboo failure in the build output of every Vite 8 project.
...
- Updated dependencies [7e366cb]
  - @bamboocss/core@1.55.7
  - @bamboocss/node@1.55.7
  - @bamboocss/config@1.55.7
  - @bamboocss/extractor@1.55.7
  - @bamboocss/logger@1.55.7
  - @bamboocss/shared@1.55.7
  - @bamboocss/ts-ast@1.55.7
...
- Updated dependencies [235afe9]
  - @bamboocss/core@1.55.6
  - @bamboocss/node@1.55.6
  - @bamboocss/config@1.55.6
  - @bamboocss/extractor@1.55.6
  - @bamboocss/logger@1.55.6
  - @bamboocss/shared@1.55.6
  - @bamboocss/ts-ast@1.55.6
...
- 5586ec9: Keep the stylesheet hot-updating when something requests it before anything imports it, which TanStack Start
  does whenever `__root.tsx` imports `virtual:bamboo.css` (#114).
  ...
  that environment. A browser's own import never meets this, because the importer creates the node first. TanStack
  Start's dev-only SSR style collection does: it requests the stylesheet from the client environment on the server,
  before the browser has asked for it.
...
- Updated dependencies [1f4ec2e]
  - @bamboocss/node@1.54.2
...
- 52e847c: Give each lazily loaded chunk a stylesheet of the utilities only it uses.
  - An atom only one lazily loaded chunk's modules emit is moved out of the entry sheet into a sheet of that chunk's
    own, attached where Vite's own plumbing reads it: the manifest lists it under the chunk, the preload helper fetches
    it before the chunk runs, and a framework that copies a build's stylesheets by the names its chunks recorded copies
    it too. An atom two chunks share, one the entry reaches, or one `staticCss` asked for stays in the entry sheet, so
...
- 7161a9f: Prune the stylesheet against every build environment, not only the one that emits it.
  ...
  - The sheet is pruned in a `pre`-ordered `generateBundle` hook, before any other plugin reads its name. A framework
    that records asset names in its own hook records the final one; `@vitejs/plugin-rsc` did not, and every
    server-rendered page linked a stylesheet that no longer existed.
...
- Updated dependencies [1352cba]
- Updated dependencies [5ea4969]
...
- Updated dependencies [a019d80]
  - @bamboocss/node@1.52.0
