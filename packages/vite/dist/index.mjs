...
const bamboocss = (options = {}) => {
	...
	*/
	...
	async function compileModule(code, id, sfcOnly) {
		...
		if (!(host.isSourceFile(filePath) && ctx.project.getSourceText(filePath) === code) && !namesEntrypoint(ctx, code) && !importsSourceModule((path) => ctx.project.sourceIsOverridden(path) || host.isSourceFile(path) && existsSync(path), (specifier) => {
