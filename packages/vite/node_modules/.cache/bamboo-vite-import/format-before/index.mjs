...
const loadNodeModule = createRetryableLazy(() => import("@bamboocss/node"));
const loadConfigModule = createRetryableLazy(() => import("@bamboocss/config"));
/**
	...
	*/
	...
	const refoldMatchesSignature = (state, dependent) => {
		...
		try {
			...
			*
			* A changed result is only a provisional check on the way to a real transform, so it must
			* not mutate the last recoverable edges. The transform records the authoritative answer;
			* if that pass throws, retaining the old edges is what lets a dependency edit retry it.
			*/
