...
A recipe is erased when it compiles, so it has one method besides being called:

- `splitVariantProps`: takes an object and returns an array containing the recipe variant props and the rest of the
  props. The compiler lowers it to a plain `splitProps` call, so it does not keep the recipe alive.
...
Any other read of the recipe — `raw`, `variantMap`, `config`, `getVariantProps`, `merge` — fails the build with
`runtime-binding` (or `raw-call` for `.raw()`), and the recipe types do not declare them. For a Storybook control, list
the values where the stories can see them:
