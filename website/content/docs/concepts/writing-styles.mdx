...
mark, so the build still reports the value — failing the build by default, exactly like `rd.400` above.
...
Grade it with [`unresolvedToken`](/docs/references/config#unresolvedtoken): a nonexistent token name like `navH` above
...
style object has no such finite set; use [dynamic styling](/docs/guides/dynamic-styling) for values that must remain
open.
...
Both examples preserve the layout and select the blue color before allocating classes. A slot of an inline `sva`
composes the same way: `cx(card({ size }).root, css({ color: 'blue.400' }))`.
...
layered CSS whatever its specificity. The case to watch is an opaque class that is _itself_ a Bamboo class — one
assembled at runtime, or arriving through a prop the compiler cannot follow — because then both sides are in
`@layer utilities`, and the sublayer order decides rather than the `cx()` argument order.

Which of the two you handed it is not something the compiler can see — an opaque string is opaque — so it reports every
such mix under `opaque-composition`, apart from an ordinary rejected call, so
[`reportSkipped`](/docs/guides/source-transformation) and the build summary can name them.
...
[hash reference](/docs/references/config#hash) for independent class-name and CSS-variable settings.
