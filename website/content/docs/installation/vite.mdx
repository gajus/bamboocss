...
Installing the published packages does not require Rust. Alpine Linux/musl and Windows arm64 have no bundled binary; use
a supported build environment. There is no JavaScript or TypeScript extraction fallback. These requirements apply to
build machines; browsers receive the compiled JavaScript and CSS.

For a missing or unloadable binary, see
[native extraction errors](/docs/concepts/build-diagnostics#native-extraction-is-unavailable).
...
from, so a compiled class always has a rule behind it. A module outside that inventory is left alone, with two
exceptions: one whose text names a bamboo entrypoint, since the build's check for classes without rules exists to report
exactly that misconfiguration, and one importing a module the inventory covers, which may be calling a recipe declared
there. Excluding generated code, as in `exclude: ['**/__generated__/**']`, therefore also keeps it out of the compiler.
