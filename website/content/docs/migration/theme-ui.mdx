...
name, so it composes with any element. Theme UI's `width` on a `Grid` is the minimum column width, which is
`minChildWidth` here. Its responsive array becomes a condition object — an array is not a style value in Bamboo — so
`[128, null, 192]` becomes `{ base: '128px', md: '192px' }`.
...
const Demo = () => (
  <div className={grid({ minChildWidth: { base: '128px', md: '192px' } })}>
